...
chrome.downloads.onCreated.addListener((downloadItem) => {
  ...
  if (isYtDlpSite(downloadItem.referrer || downloadItem.url || "") || downloadItem.url.startsWith("blob:")) {
    // Let the browser handle it normally — yt-dlp sites are handled via the popup instead.
  ...
  // Cancel as early as possible — onCreated fires at download start, before
  // Firefox/Chrome have written significant data, giving cancel() a real chance to work.
  chrome.downloads.cancel(downloadItem.id, () => {
    if (chrome.runtime.lastError) {
      // Already finished/cancelled — harmless race, ignore.
    }
...
chrome.downloads.onDeterminingFilename.addListener((downloadItem, suggest) => {
  // onCreated above already handles cancellation. This listener now only
  // needs to let the browser proceed normally for anything onCreated skipped
  // (e.g. yt-dlp sites), since Firefox/Chrome still expect a response here.
  suggest();
});
